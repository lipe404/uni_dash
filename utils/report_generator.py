...
class ReportGenerator:
    ...
    def generate_summary_report_excel(self, parceiro_nome: str, ano: int = None, mes: int = None, modalidades: List[str] = None) -> bytes:
        ...
        try:
            df_vendas = self.get_filtered_sales_data(parceiro_nome, ano, mes, modalidades)
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                resumo_data = {
                    ...
                    'Valor': [
                        ...
                        df_vendas['IES'].nunique() if 'IES' in df_vendas.columns else 0,
                ...
                modalidades_count = modalidades_count.sort_values('Qtd. Matrículas', ascending=False)
                ...
                if 'Valor Pagto' in df_vendas.columns:
                    ...
                    for modalidade in modalidades_count['Nível']:
                        ...
                        valores_modalidade.append(self.format_currency_value(valor_mod))
                    modalidades_count['Valor Total'] = valores_modalidade

                modalidades_count.columns = ['Modalidade', 'Total de Matrículas'] + (['Valor Total'] if 'Valor Pagto' in df_vendas.columns else [])
                modalidades_count.to_excel(writer, sheet_name='Por Modalidade', index=False)
                ...
                cursos_count = cursos_count.sort_values('Qtd. Matrículas', ascending=False)
                ...
                if 'Valor Pagto' in df_vendas.columns:
                    ...
                    for curso in cursos_count['Curso']:
                        ...
                        valores_curso.append(self.format_currency_value(valor_curso))
                    cursos_count['Valor Total'] = valores_curso

                cursos_count.columns = ['Curso', 'Total de Matrículas'] + (['Valor Total'] if 'Valor Pagto' in df_vendas.columns else [])
                cursos_count.to_excel(writer, sheet_name='Por Curso', index=False)
                ...
                if 'IES' in df_vendas.columns:
                    ...
                    ies_count = ies_count.sort_values('Qtd. Matrículas', ascending=False)
                    ...
                    if 'Valor Pagto' in df_vendas.columns:
                        ...
                        for ies in ies_count['IES']:
                            ...
                            valores_ies.append(self.format_currency_value(valor_ies))
                        ies_count['Valor Total'] = valores_ies

                    ies_count.columns = ['IES', 'Total de Matrículas'] + (['Valor Total'] if 'Valor Pagto' in df_vendas.columns else [])
                    ies_count.to_excel(writer, sheet_name='Por IES', index=False)
                ...
                if ano:
                    ...
                    vendas_mensais['Nome do Mês'] = vendas_mensais['Mês'].map(meses_nomes)
                    ...
                    if 'Valor Pagto' in df_vendas.columns:
                        ...
                        for mes_num in vendas_mensais['Mês']:
                            ...
                            valores_mes.append(self.format_currency_value(valor_mes))
                        vendas_mensais['Valor Total'] = valores_mes

                    vendas_mensais = vendas_mensais[['Nome do Mês', 'Qtd. Matrículas'] + (['Valor Total'] if 'Valor Pagto' in df_vendas.columns else [])]
                    vendas_mensais.columns = ['Mês', 'Total de Matrículas'] + (['Valor Total'] if 'Valor Pagto' in df_vendas.columns else [])
                    vendas_mensais.to_excel(writer, sheet_name='Por Mês', index=False)
    ...
    def generate_detailed_report_excel(self, parceiro_nome: str, ano: int = None, mes: int = None, modalidades: List[str] = None) -> bytes:
        ...
        try:
            ...
            colunas_export = ['Parceiro', 'Aluno', 'Nível', 'Curso', 'IES', 'Dt Pagto', 'Qtd. Matrículas', 'Valor Pagto']

            # Verificar quais colunas existem no DataFrame
            colunas_disponiveis = [col for col in colunas_export if col in df_vendas.columns]

            df_export = df_vendas[colunas_disponiveis].copy()
            df_export['Dt Pagto'] = df_export['Dt Pagto'].dt.strftime('%d/%m/%Y')
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                df_export.to_excel(writer, sheet_name='Dados Detalhados', index=False)
                ...
                resumo_data = {
                    ...
                    'Valor': [
                        len(df_export),
                        int(df_export['Qtd. Matrículas'].sum()) if 'Qtd. Matrículas' in df_export.columns else 0,
                        self.format_currency_value(valor_total),
                        f"{ano if ano else 'Todos os anos'} - {mes if mes else 'Todos os meses'}",
                        ', '.join(modalidades) if modalidades and "Todas" not in modalidades else "Todas",
    ...
    def generate_csv_report(self, parceiro_nome: str, ano: int = None, mes: int = None, modalidades: List[str] = None, detailed: bool = False) -> bytes:
        ...
        try:
            ...
            if detailed:
                ...
                # Relatório resumido
                df_export = df_vendas.groupby(['Nível', 'Curso'])['Qtd. Matrículas'].sum().reset_index()
                df_export = df_export.sort_values('Qtd. Matrículas', ascending=False)
    ...
    def generate_pdf_report(self, parceiro_nome: str, ano: int = None, mes: int = None, modalidades: List[str] = None, detailed: bool = False) -> bytes:
        ...
        try:
            ...
            story.append(Paragraph(f"Relatório de Vendas - {parceiro_nome}", title_style))
            ...
            info_data = [
                ['Período:', f"{ano if ano else 'Todos os anos'} - {mes if mes else 'Todos os meses'}"],
                ['Modalidades:', ', '.join(modalidades) if modalidades and "Todas" not in modalidades else "Todas"],
                ['Data de Geração:', datetime.now().strftime('%d/%m/%Y %H:%M')],
                ['Total de Vendas:', str(len(df_vendas))],
                ['Total de Matrículas:', str(int(df_vendas['Qtd. Matrículas'].sum()))],
            ...
            if detailed:
                ...
                story.append(Paragraph("Detalhamento das Vendas (Primeiras 30 linhas)", styles['Heading2']))
                ...
                for _, row in df_limited.iterrows():
                    valor_formatado = self.format_currency_value(row.get('Valor Pagto', ''))
                    data.append([
                        row['Aluno'][:15] + '...' if len(str(row['Aluno'])) > 15 else str(row['Aluno']),
                        str(row['Nível'])[:15] + '...' if len(str(row['Nível'])) > 15 else str(row['Nível']),
                        row['Curso'][:20] + '...' if len(str(row['Curso'])) > 20 else str(row['Curso']),
                        row['IES'][:15] + '...' if 'IES' in row and len(str(row['IES'])) > 15 else str(row.get('IES', 'N/A')),
                        row['Dt Pagto'].strftime('%d/%m/%Y'),
                        valor_formatado[:10] + '...' if len(valor_formatado) > 10 else valor_formatado
                    ])

                table = Table(data, colWidths=[1*inch, 0.8*inch, 1.2*inch, 0.8*inch, 0.7*inch, 0.8*inch])

            else:
                # Tabela resumida por modalidade
                story.append(Paragraph("Resumo por Modalidade", styles['Heading2']))
                story.append(Spacer(1, 10))

                modalidades_summary = df_vendas.groupby('Nível')['Qtd. Matrículas'].sum().reset_index()
                modalidades_summary = modalidades_summary.sort_values('Qtd. Matrículas', ascending=False)

                data = [['Modalidade', 'Total de Matrículas']]
                for _, row in modalidades_summary.iterrows():
                    data.append([str(row['Nível']), str(int(row['Qtd. Matrículas']))])
